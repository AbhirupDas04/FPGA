...
void chacha(uint32_t plaintext[][16],uint32_t key[8],uint32_t nonce[3],uint32_t counter,uint32_t cipher[][16],uint32_t act_state_matrix[4][4]);
...
//		printf("%08x\n",nonce[i]);
	...
	chacha(plaintext,key,nonce,counter_val,cipher,act_state_matrix);

	uint32_t key_1[8] = {0x1c9240a5,0xeb55d38a,0xf3338886,0x04f6b5f0,0x473917c1,0x402b8009,0x9dca5cbc,0x207075c0};
	uint32_t nonce_1[3]= {0x00000000,0x00000000,0x00000002};

    // uint32_t key_1[8] = {0x00000000,0x00000000,0x00000000,0x00000000,0x00000000,0x00000000,0x00000000,0x00000000};   //8*32=256 bits keys 
    // uint32_t nonce_1[3]= {0x00000000,0x00000000,0x00000000};

    // uint32_t key_1[8] = {0x00010203,0x04050607,0x08090a0b,0x0c0d0e0f,0x10111213,0x14151617,0x18191a1b,0x1c1d1e1f};   //8*32=256 bits keys 
    // uint32_t nonce_1[3]= {0x00000000,0x0000004a,0x00000000}  ;
...
//		printf("%08x\n",nonce[i]);
	...
	for(int i = 0; i < 4; i++){
		local_stream.data = act_state_matrix[0][i];
	...
	for(int i = 0; i < n_blocks; i++){
        for(int j = 0; j < 16; j++){
            local_stream.data = plaintext[i][j];
...
//printf("%08x\n",key_1[i]);
	...
	for(int i = 0; i < n_blocks; i++){
        for(int j = 0; j < 16; j++){
            ...
            cipher_hw[i][j]=local_stream.data;
	...
	for(int i = 0; i < n_blocks; i++){
        for(int j = 0; j < 16; j++){
        	//printf("%08x ",cipher_hw[i][j]);
            if(cipher[i][j]!=cipher_hw[i][j]){
            	//printf("%08x %08x",cipher[i][j],cipher_hw[i][j]);
                printf("Error\n");
            }
